...
def delete_community(name):
    ...
    community = next((c for c in communities if c["name"].lower() == name.lower()), None)
    if community:
        communities = [c for c in communities if c["name"].lower() != name.lower()]
        return jsonify({"message": f"Community '{name}' deleted successfully"}), 200
    return jsonify({"error": "Community not found"}), 404
...
def delete_job(name):
    ...
    job = next((j for j in jobs if j["name"].lower() == name.lower()), None)
    if job:
        jobs = [j for j in jobs if j["name"].lower() != name.lower()]
...
def delete_translation(name):
    ...
    translation = next((t for t in translations if t["name"].lower() == name.lower()), None)
    if translation:
        translations = [t for t in translations if t["name"].lower() != name.lower()]
