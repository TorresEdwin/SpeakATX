...
const Home = () => {
  ...
  useEffect(() => {
    ...
    if (savedData) {
      // If data is found in localStorage, parse it and set it to state
      setPieChartData(JSON.parse(savedData));
    } else {
      // If no saved data, calculate and save it to localStorage
      ...
      // Convert the language counts to pie chart data format
      const newPieChartData = Object.keys(languageCounts).map(language => ({
        name: language.charAt(0).toUpperCase() + language.slice(1),
        value: languageCounts[language],
      }));

      // Save the new data to localStorage and set it in state
      localStorage.setItem("pieChartData", JSON.stringify(newPieChartData));
